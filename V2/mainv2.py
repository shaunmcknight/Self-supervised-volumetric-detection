...
def setup_dataloaders(training_data, validation_data, test_data):    
    ...
    valid_dataloader= DataLoader(
        CleanDataset(data=validation_data, transforms=None, label='Validation'),
        batch_size=hp.batch_size,
        shuffle=False,
    )
    
    test_dataloader= DataLoader(
        CleanDataset(data=test_data, transforms=None, label='Test'),
        batch_size=hp.batch_size,
        shuffle=False,
    )
...
training_arrays = [x[0:260,:,:] for x in training_arrays] # cut down number of frames to match
time_limits = [600, 450, 450]   #set time limits to just past backwall
...
validation_arrays = ['ID004']
validation_arrays = [np.load(os.path.join(base_dir, x, x+'_hilbert_raster.npy')) for x in validation_arrays]
validation_arrays = [x[0:260,:,:] for x in validation_arrays] # cut down number of frames to match
time_limits = [350]   #set time limits to just past backwall

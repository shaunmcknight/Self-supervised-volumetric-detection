...
class CNN_shared(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.gap(out)
        # print(out.size())
        out = self.linear(self.linear_hidden(out.squeeze().squeeze()))
        mean = out[:,0]
        std = out[:,1]
        return torch.abs(mean), torch.abs(std)
    
class CNN_shared_relu(nn.Module):
    def __init__(self):
        super(CNN_shared_relu, self).__init__()
        self.c1=32
        self.c2=64
        self.c3=128
    ...
    def forward(self, x):
        ...
        out = self.gap(out)
        # print(out.size())
        out = self.linear(self.relu(self.linear_hidden(out.squeeze().squeeze())))
        mean = out[:,0]
        std = out[:,1]
        return torch.abs(mean), torch.abs(std)
    
class CNN_shared_new(nn.Module):
    def __init__(self):
        super(CNN_shared_new, self).__init__()
        self.c1=32
        self.c2=64
        self.c3=128
